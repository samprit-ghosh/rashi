...
import { generateHoroscope, testGeminiAPI, getKeyStatus } from '../../utils/geminiClient';
import { rashiData } from '../../utils/astrologyCalculations';
import { Calendar, Heart, Briefcase, Activity, Target, Palette, Sparkles, RefreshCw, AlertCircle, Key, Users, Cpu, Zap, Timer, ChevronLeft, ChevronRight, Play, Pause, Star, Sparkle, Moon, Sun } from 'lucide-react';
...
const RashiWheelHoroscope = () => {
  ...
  const fetchHoroscope = async () => {
    ...
    updateGenerationStatus('Starting AI generation...', 'Initializing', 5);

    try {
      updateGenerationStatus('Preparing request...', 'Setup', 10);
      await new Promise(resolve => setTimeout(resolve, 300));

      updateGenerationStatus('Selecting API key...', 'Key Selection', 20);
      await new Promise(resolve => setTimeout(resolve, 200));

      updateGenerationStatus('Choosing AI model...', 'Model Selection', 30);
      await new Promise(resolve => setTimeout(resolve, 200));

      updateGenerationStatus('Sending to Gemini AI...', 'API Request', 50);
      await new Promise(resolve => setTimeout(resolve, 300));

      const prediction = await generateHoroscope(selectedRashi);

      updateGenerationStatus('Processing AI response...', 'Processing', 80);
      await new Promise(resolve => setTimeout(resolve, 400));

      setHoroscope(prediction);
      updateGenerationStatus('Generation complete!', 'Complete', 100);

      // Scroll to top when new horoscope is generated
      setTimeout(() => {
        stopTimer();
        setGenerationStatus('');
        setCurrentStep('');
          ...
          <div className="lg:col-span-7 h-[110vh]" ref={rightSectionRef}>
            <div className="bg-black/40 backdrop-blur-2xl rounded-2xl sm:rounded-3xl border border-white/20 shadow-2xl relative overflow-hidden h-full flex flex-col min-h-[600px]">
              ...
              <div className="text-center p-4 sm:p-6 lg:p-8 border-b border-white/20 relative z-10 bg-black/20 backdrop-blur-sm">
                ...
                <p className="text-blue-200 text-sm sm:text-lg">AI-Powered Celestial Guidance</p>
              </div>
                ...
                {!selectedRashi ? (
                  ...
                  <div className="content-container h-full overflow-y-auto">
                    <div className="flex flex-col p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
                      {/* Generate Button - Fixed positioning removed, part of scroll */}
                      ...
                      {horoscope && (
                        <div className="space-y-4 sm:space-y-6 animate-fadeIn">
                          <div className="text-center">
                            <span className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full text-sm sm:text-base lg:text-lg font-bold shadow-2xl border border-white/20">
                              ✨ Celestial Insights for {selectedRashi}
                            </span>
                            <div className="flex flex-col sm:flex-row items-center justify-center space-y-1 sm:space-y-0 sm:space-x-6 mt-2 sm:mt-4">
                              <p className="text-green-300 text-xs sm:text-sm flex items-center">
                                <Timer className="w-4 h-4 mr-1 sm:mr-2" />
                                Cosmic Computation: <span className="font-mono font-bold ml-1 sm:ml-2 text-xs sm:text-sm">{formatTime(generationTime)}</span>
                              </p>
                            </div>
                          </div>

                          {/* Lucky Elements */}
                          {(horoscopeSections.luckyNumber || horoscopeSections.luckyColor) && (
                            <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 shadow-2xl backdrop-blur-lg">
                              <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-4 sm:mb-6 text-center">🎯 Today's Celestial Blessings</h3>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
                                {horoscopeSections.luckyNumber && (
                                  <div className="text-center bg-black/30 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-2xl border border-white/20 backdrop-blur-sm">
                                    <Target className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-yellow-400 mx-auto mb-2 sm:mb-3" />
                                    <p className="text-yellow-200 text-xs sm:text-sm font-medium mb-1 sm:mb-2">Lucky Number</p>
                                    <p className="text-xl sm:text-xl lg:text-xl font-bold text-yellow-400 mt-1">{horoscopeSections.luckyNumber}</p>
                                  </div>
                                )}
                                {horoscopeSections.luckyColor && (
                                  <div className="text-center bg-black/30 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-2xl border border-white/20 backdrop-blur-sm">
                                    <Palette className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-orange-400 mx-auto mb-2 sm:mb-3" />
                                    <p className="text-orange-200 text-xs sm:text-sm font-medium mb-1 sm:mb-2">Lucky Color</p>
                                    <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-orange-400 mt-1">{horoscopeSections.luckyColor}</p>
                                  </div>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="text-center pt-4 sm:pt-6 border-t border-white/20 pb-8">
                            <p className="text-cyan-300 text-xs sm:text-sm font-medium">
                              ✨ Channeled in {formatTime(generationTime)} through Cosmic AI Network
                            </p>
                          </div>
                        </div>
                      )}
